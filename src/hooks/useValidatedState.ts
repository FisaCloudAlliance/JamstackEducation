...
export const useValidatedState = <T>(validator, initialValue?: T) => {
  ...
  const _validation = useRef<validation>({
    changed: false, errorMessage: "", hasError: false
  });

  const onChange = useCallback((nextValue) => {
    setState(nextValue);
    const valid = validator(nextValue);
    _validation.current = { ..._validation.current, changed: true, errorMessage: valid, hasError: !!valid };
  }, [_validation]);
  ...
  return { state, onChange, validate, ...validation };
};
...
export type validateState = validator & validation & {
  state: any,
  onChange: (v: any) => void,
}